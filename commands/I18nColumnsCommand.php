...
class I18nColumnsCommand extends CConsoleCommand
{
    ...
    public $modelsSubPath = '.ar';
    
    public $modelsToExclude = array('ActiveRecord');
    /**
     * @var array
     */
    ...
    protected function _loadLanguages()
    {
        ...
        if (!isset($config['components']['multilingual']['languages'])) {
        ...
        $this->languages = $config['components']['multilingual']['languages'];
...
class ' . $migrationName . ' extends CDbMigration
{
    ...
    protected function _getModels()
    {
        $models = array();
        $aliases = array();
        $aliases[] = 'application.models'.$this->modelsSubPath;
        foreach (Yii::app()->getModules() as $moduleName => $config) {
            if ($moduleName != 'gii') {
                $aliases[] = $moduleName . ".models";
                if ($this->modelsSubPath) {
                    $aliases[] = $moduleName . '.models.'.$this->modelsSubPath;
                }
            }
        }
        ...
        foreach ($aliases as $alias) {
            ...
            foreach ($files as $file) {
                ...
                if ($fileClassName = $this->_checkFile($file, $alias)) {
                    ...
                    if (in_array($fileClassName, $this->modelsToExclude)) {
                        continue;
                    }
